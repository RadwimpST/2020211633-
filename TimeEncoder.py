...
class PatchTST(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Positional encoding (learnable or none)
        self.pe_type = pe
        if pe == "learnable":
            ...
            raise ValueError("pe must be 'learnable' or 'none'")
        self.pos_drop = nn.Dropout(dropout) if dropout and dropout > 0 else nn.Identity()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # If actual L differs (e.g., different T at runtime), adjust PE safely
        if self.pe_type == "learnable":
            if L != self.pos_embed.size(1):
                # Interpolate position embeddings along the token dimension
                pos = F.interpolate(
                    self.pos_embed.transpose(1, 2), size=L, mode="linear", align_corners=False
                ).transpose(1, 2)
            else:
                pos = self.pos_embed
            tokens = tokens + pos
        ...
        # Reshape back to (B, out_channels, V)
        feat = feat.view(B, V, M, self.out_channels).permute(0, 3, 1, 2).contiguous()
        feat = feat.mean(dim=3)  # average over M if M>1 (kept for API parity)
        print(feat.shape)
        return feat
