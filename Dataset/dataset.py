...
from .DataLoaders.dataLoader import abide1Loader, ADHDLoader
...
def getDataset_test(options):
    return SupervisedDataset_test(options)
...
class SupervisedDataset_test(Dataset):
    ...
    def setFold(self, fold, train=True):
        ...
        self.randomRanges = [[np.random.randint(0, self.data[idx].shape[-1] - self.dynamicLength) for k in range(9999)] for idx in testIdx]
    ...
    def __getitem__(self, idx):
        ...
        std = np.std(timeseries[0:400,:], axis=1, keepdims=True)

        epsilon = 1e-8
        std = np.where(std < epsilon, 1.0, std)

        timeseries[0:400,:] = (timeseries[0:400,:] - np.mean(timeseries[0:400,:], axis=1, keepdims=True)) / std
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, datasetDetails):
        ...
        self.kFold = StratifiedKFold(datasetDetails.foldCount, shuffle=False,
                                     random_state=None) if datasetDetails.foldCount is not None else None
        self.k = None
    ...
    def setFold(self, fold, train=True):
        ...
        self.targetData = [self.data[idx] for idx in trainIdx] if train else [self.data[idx] for idx in testIdx]
        self.targetLabels = [self.labels[idx] for idx in trainIdx] if train else [self.labels[idx] for idx in testIdx]
        self.targetSubjIds = [self.subjectIds[idx] for idx in trainIdx] if train else [self.subjectIds[idx] for idx in
                                                                                       testIdx]

        if (train and not isinstance(self.dynamicLength, type(None))):
            np.random.seed(self.seed + 1)
            self.randomRanges = [
                [np.random.randint(0, self.data[idx].shape[-1] - self.dynamicLength) for k in range(9999)] for idx in
                trainIdx]
